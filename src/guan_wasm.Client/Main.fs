module guan_bolero.Client.Main
...
let solve () =
  ...
  try
    let guanMod = Module.Parse(
      "persontests", ResizeArray [], null
    )
    ()
  with exn -> Console.WriteLine $"Exn: {exn}"

  ()

  // let moduleProvider = ModuleProvider ()
  // moduleProvider.Add guanMod

  // let queryContext = QueryContext moduleProvider
  
  // let goal = "person(?x)"
  // let query = Query.Create(goal, queryContext)
  // task {
  //   let! result = query.GetNextAsync ()
